                    ...
                    <li>
                        ...
                        {isSidebarOpen && (
                            <div className='navbar-nav-dropdown'>
                                <button onClick={() => { setIsSidebarOpen(!isSidebarOpen) }}>{isLoaged ? <h5 onClick={logout}> Logout </h5> :
                                <Link style={{ textDecoration: 'none' }} to='/Login' > <h5 onClick={reload}  >Login</h5></Link>}</button>
                                <Link to='/login'><button onClick={() => { setIsSidebarOpen(!isSidebarOpen) }}>User</button></Link>
                                <Link to='/seller'><button onClick={() => { setIsSidebarOpen(!isSidebarOpen) }}>Sellers</button></Link>
                                <Link to='/admin'><button onClick={() => { setIsSidebarOpen(!isSidebarOpen) }}>Admin panel</button></Link>
                            </div>
                        )}
                    </li>
                </ul>
